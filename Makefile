...
$(BINDIR)/$(BINNAME): $(SRC)
	GO111MODULE=on go build $(GOFLAGS) -tags '$(TAGS)' -ldflags '$(LDFLAGS)' -o $(BINDIR)/$(BINNAME) $(BINNAME).go
...
.PHONY: format
format: $(GOIMPORTS)
	GO111MODULE=on go list -f '{{.Dir}}' ./... | xargs $(GOIMPORTS) -w -local github.com/fabito/azure-storage-purger
...
dist:
	( \
		...
		$(DIST_DIRS) tar -zcf azp-${VERSION}-{}.tar.gz {} \; && \
		$(DIST_DIRS) zip -r azp-${VERSION}-{}.zip {} \; \
	)
